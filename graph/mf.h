...
namespace net
{
	...
	namespace cost_flow
	{
		...
		pair<wT,cT> mcmf_spfa(const vector<tuple<int,int,wT,cT>> &edges,int S,int T)//[0,n]
		{
			...
			pair<wT,cT> r{0,0},rr;
			while ((rr=spfa()).first&&rr.second<0) r={r.first+rr.first,r.second+rr.second};
			return r;
		}
...
int main()
{
	...
	vector<int> rd(n+1);
	i=0;
	for (auto &[u,v,w,c]:edges) cin>>u>>v>>w>>c,rd[u]+=w&1,rd[v]-=w&1,ans[i++]=w&1,w>>=1;
	ll tot=0;
	for (i=2; i<n; i++)
	{
		if (rd[i]%2) { cout<<"Impossible\n"; return 0; }
		rd[i]/=2;
		if (rd[i]>0)
		{
	...
	// dbg(W,C);
	if (C>tot+500'000'000) { cout<<"Impossible\n"; return 0; }
